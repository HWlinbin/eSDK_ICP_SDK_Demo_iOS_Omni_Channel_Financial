...
- (void)viewDidLoad {
    ...
    [[CCSDK shareInstance] setAnonymousCard:@"AnonymousCard"];
...
//                               content:[NSString stringWithFormat:@"%@ %@:%@",NSLocalizedString(@"Chat", ""),NSLocalizedString(@"Fail", ""),result] delegate:self];
              [CCDemoUtil showAlertWithTitle:NSLocalizedString(@"Remind", "")
                                     content:[NSString stringWithFormat:@"%@ %@:%@",NSLocalizedString(@"Chat", ""),NSLocalizedString(@"Fail", ""),result]];
          });
    }
    else
    {
...
- (void)cameraSwitchClick
{
    ...
    [self performSelector:@selector(cameraSwitch) withObject:nil afterDelay:0.1];
}
...
- (void)cameraSwitch
{
    ...
    if (_isBackCamera)
    {
        if([[CCSDK shareInstance] switchCamera:1])
        {
            _isBackCamera = NO;
    ...
    if([[CCSDK shareInstance] switchCamera:0])
    {
        _isBackCamera = YES;
