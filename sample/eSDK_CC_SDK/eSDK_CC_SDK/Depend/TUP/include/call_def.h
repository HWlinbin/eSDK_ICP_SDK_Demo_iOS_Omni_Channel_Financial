...
#define TUP_CALL_VERSION                 "TUP V600R006C00SPC300B010"
...
#define CALL_D_MAX_CONF_SUBJECT_LEN (128) /**< [en]Indicates the max length of the conference subject  
                                                    <br>[cn]会议主题最大长度 */
...
#define CALL_D_CFG_MEDIA_RTP_KEEPALIVE_INTERVAL    CFG_ID_DEF(CALL_E_CFG_MEDIA, CALL_E_CFG_DATA_TYPE_INT,           0x15, 0) /**< [en]Indicates global media configuration: Configure the empty packet send interval of RTP
                                                                                                                                      <br>[cn]全局媒体配置:辅流停发时，发送RTP空包保活端口间隔间隔, 默认25s (单位:秒)*/
...
#define CALL_D_CFG_AUDIO_OPENSLES_MODE             CFG_ID_DEF(CALL_E_CFG_AUDIO, CALL_E_CFG_DATA_TYPE_INT,           0x27,0)  /**< [en]audio config: set OpenSLES mode to enable VQE(for Andruid Mobile only) 0 means disabled 1 enabled
                                                                                                                                  <br>[cn]音频配置:设置jitter buf防抖动参数, 对应的结构体为CALL_S_JITBUF_PARAMS 
                                                                                                                                  <br>[cn]音频配置:设置OpenSLES模式，只适用Android Mobile。是否启用系统VQE，0表示不启用系统VQE，1表示启用系统VQE*/
#define CALL_D_CFG_AUDIO_NETATE_REDPT_SWITCH       CFG_ID_DEF(CALL_E_CFG_AUDIO, CALL_E_CFG_DATA_TYPE_BOOL,          0x28,0)  /**< 音频配置:netate开启，冗余PT值是否采用RED PT值，若此开关打开了，冗余PT值为RED PT值，若关闭冗余PT 为127，默认关闭>**/
...
#define CALL_D_CFG_VIDEO_RESOLUTION_RANGE_SHIFT    CFG_ID_DEF(CALL_E_CFG_VIDEO, CALL_E_CFG_DATA_TYPE_INT,           0x24, 0) /**< [en]Indicates video configuration: resolution range shift[0-3] when auto adjust bitrate.Default is 0, means disable CALL_S_RESOLUTION_TABLE table. [1,3] means, TUP will auto adjust resolution by band width in CALL_S_RESOLUTION_TABLE. It is valid when CALL_D_CFG_VIDEO_ARS is TRUE。
                                                                                                                                  <br>[cn]视频配置:自动调速时，分辨率可调范围n，n可设置为[0-3]。默认为0，表示不启用分辨率表CALL_S_RESOLUTION_TABLE。 [1,3]，根据会话协商分辨率，从分辨率表CALL_S_RESOLUTION_TABLE内，找到对应的分辨率档位，n表示从该档位开始，可以往下调整n-1档。CALL_D_CFG_VIDEO_ARS为TRUE时才有效。*/
