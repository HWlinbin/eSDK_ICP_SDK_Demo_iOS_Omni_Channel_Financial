...
- (void)webChatCallWithAccessCode:(NSString *)accessCode callData:(NSString *)callData verifyCode:(NSString *)verifyCode
{
    ...
    logDbg(@"web chat call start create chat connection");
...
- (void)makeCallConnection:(NSString *)accessCode callType:(NSString *)callType callData:(NSString *)callData verifyCode:(NSString *)verifyCode
{
    ...
    else
    {
        ...
        postDict = @{MEDIA_TYPE_KEY:@2,CALLER_KEY:[CCAccountInfo shareInstance].userName,ACCESS_CODE_KEY:accessCode,CALL_DATA_KEY:callData,UVID_KEY:uvidNum,VERIFY_CODE_KEY:verifyCode};
        NSData *postData = [NSJSONSerialization dataWithJSONObject:postDict options:NSJSONWritingPrettyPrinted error:nil];
...
- (void)handleResponseHeader:(NSHTTPURLResponse *)httpResponse
{
    ...
    if (httpResponse.statusCode == 200)
    {
        if ([[CCAccountInfo shareInstance].loginPath isEqualToString:relativePath])
        {
            ...
            _cooKie = [[headFieldDict objectForKey:SET_COOKIE_KEY] substringFromIndex:11];
    ...
    else if ([eventType isEqualToString:PREPARE_JOIN_MEETING_VALUE])
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (!confRet)
            {
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:CALL_MSG_ON_FAIL object:createFail];
            }
            else
            {
