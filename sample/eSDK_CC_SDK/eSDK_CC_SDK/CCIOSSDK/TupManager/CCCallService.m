...
{
    logDbg(@"applicationWillResignActiveNotification");
    _isbackGround = YES;
    [self videoOperate:STOP|CLOSE];
}
- (void)applicationDidBecomeActiveNotification
{
    ...
    logDbg(@"applicationDidBecomeActiveNotification");
                 ...
                 videoOperation:(EN_VIDEO_OPERATION)operation
{
    ...
    [self setCaptureDisplayAndMirrorType:self.callID renderWindow:CALL_E_VIDEOWND_CALLLOCAL displayType:0 mirrorType:mirrorType];
}
                                  ...
                                  data:(void *)data
{
    ...
    switch (notifyEvent)
    {
        ...
        {
            [((UIView *)self.localViewWindow) setNeedsDisplay];
        }
            break;
        case CALL_E_EVT_DECODE_SUCCESS:
        {
            [((UIView *)self.remoteViewWindow) setNeedsDisplay];
        }
            break;
...
- (void)setCfg
{
    ...
    TUP_RESULT ret = tup_call_media_set_video_index((TUP_UINT32)_cameraIndex);
    logDbg(@"tup_call_media_set_video_index %d : result : %d",_cameraIndex,ret);
...
- (void)startAnonymousCallWithAccessNumber:(NSString *)accessNumber andIp:(NSString *)ip andPort:(NSString *)port
{
    ...
    logDbg(@"tup_call_set_cfg CALL_D_CFG_SIP_ANONYMOUSNUM :%@ :%d",callNum,anRet);
...
- (void)configLocalView:(id)localView remoteView:(id)remoteView callId:(unsigned int)callId;
{
    ...
    videoInfo[0].ulDisplayType = CALL_E_VIDEOWND_DISPLAY_ZOOM;
    videoInfo[1].ulVideoWndType = CALL_E_VIDEOWND_CALLREMOTE;
    videoInfo[1].ulRender = (TUP_UPTR)remoteView;
    videoInfo[1].ulDisplayType = CALL_E_VIDEOWND_DISPLAY_ZOOM;
