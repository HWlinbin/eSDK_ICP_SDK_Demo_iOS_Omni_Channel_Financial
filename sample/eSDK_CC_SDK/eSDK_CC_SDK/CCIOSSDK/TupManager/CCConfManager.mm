...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        _isSwitchCamera = YES;
        NSLog(@"init _isSwitchCamera YES");
    }
    return self;
}
...
- (void)uninitInfo
{
    ...
    _remoteAttached = NO;
    _isSwitchCamera = YES;
    NSLog(@"uninitInfo _isSwitchCamera YES");
                                 ...
                                 nSize:(TUP_INT)nSize
{
    ...
    switch (nType)
    {
        ...
        case CONF_MSG_ON_CONFERENCE_TERMINATE://会议结束通知
        {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:CALL_MSG_ON_DISCONNECTED object:VIDEO_CALL];
...
-(void)confSendData:(TUP_UINT32)deviceID
{
    //TUP_RESULT  tup_conf_send_data(IN CONF_HANDLE handle, IN TUP_UINT32 userid, IN TUP_UINT8 msg_id, IN TUP_UINT8 * pbuffer, IN TUP_UINT32 len);
    NSMutableDictionary *data = [[NSMutableDictionary alloc] init];
    NSMutableDictionary *address = [[NSMutableDictionary alloc] init];
    

    [address setObject:@(_selfUserID) forKey:@"ConferenceId"];
    [address setObject:@"false" forKey:@"IsThirdParty"];
    [address setObject:@"true" forKey:@"IsUser"];
    [address setObject:@"" forKey:@"PhoneId"];
    [address setObject:@"" forKey:@"extDeviceId"];
    [address setObject:@(deviceID) forKey:@"mainDeviceId"];
    
    [data setObject:address forKey:@"Address"];
    [data setObject:@(1) forKey:@"CallType"];
    [data setObject:@"true" forKey:@"IsAnonymous"];
    [data setObject:@"1,2,3" forKey:@"MediatypeInfo"];
    [data setObject:@(1) forKey:@"VideoType"];
    
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:data options:NSJSONWritingPrettyPrinted error:nil];
    
    TUP_RESULT rst =tup_conf_send_data(_confHandle,_confUserID,25,(TUP_UINT8 *)[jsonData bytes],(int)[jsonData length]);
    logDbg(@"confSendData rst %d,%d",rst,_localCameraInfo.deviceID);
}
...
//    入会者信息存到数组中
    ...
    if (joinUserInfo->user_alt_id != _selfUserID) {
        _confUserID = joinUserInfo->user_alt_id;
    }
...
- (void)handleComponentCallBackWithType:(TUP_INT)nType Value1:(TUP_UINT)value1 Value2:(TUP_ULONG)value2 data:(void *)data size:(TUP_INT)size
{
    switch (nType)
    {
        ...
        case COMPT_MSG_DS_ON_CURRENT_PAGE_IND:  //同步翻页预先通知
        {
            ...
            break;
        case COMPT_MSG_DS_ON_CURRENT_PAGE:  //同步翻页成功
        {
...
- (BOOL)switchCameraWithIndex:(int)index
{
    ...
    NSLog(@"switchCameraWithIndex _isSwitchCamera %d",_isSwitchCamera);
    if (!_isSwitchCamera)
    {
          return NO;
    }
    ...
    _isSwitchCamera = NO;
    NSLog(@"switchCameraWithIndex _isSwitchCamera NO");
...
{
    TUP_RESULT attachRet =  tup_conf_video_attach(_confHandle, cameraInfo.userID, cameraInfo.deviceID, (__bridge void *)cameraInfo.videoView, true,0);
